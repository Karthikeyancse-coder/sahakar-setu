...
export const JobOpportunities: React.FC = () => {
  const { jobs: contextJobs, jobInterests, applyForJob, currentUser, navigate, t } = useApp();
  const [jobs, setJobs] = useState<JobPosting[]>(contextJobs);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedSkill, setSelectedSkill] = useState<string>('all');
  const [selectedJobForModal, setSelectedJobForModal] = useState<JobPosting | null>(null);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  // Fetch real personalized match percentages from backend
  useEffect(() => {
    let isMounted = true;
    api.jobs
      .list()
      .then(fetchedJobs => {
        if (isMounted && Array.isArray(fetchedJobs) && fetchedJobs.length > 0) {
          setJobs(fetchedJobs);
        }
      })
      .catch(err => {
        console.warn('Using context jobs fallback:', err);
      });
